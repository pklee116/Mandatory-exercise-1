...
You can modify your code block easily. Try it out:

-   You can remove all the messages in the top by writing `#| message: FALSE` in the code block

-   You can remove warnings using ``` #|``warning: FALSE ```

-   You can hide the code block by writing `#| echo: FALSE` in the code block

-   You can include error messages using `#| error: TRUE` (they are FALSE by default as the render process stops if there is an error message.

-   You can exclude everything using `#| include: FALSE`

-   You can stop the code from being executed by using `#| eval: FALSE`

-   You can ensure that the results is included in future renders (ie if the code takes a long time to run) using `#| cache: TRUE`

Try it out on this first. You can check the rendered output.
...
If you want to run the code below as you are rendering, remove the chunk option `#|eval=FALSE` from the chunk below.
...
-   [Data Import Cheat Sheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_data-import.pdf),

-   [Data Transformation Cheat Sheet](https://nyu-cdsc.github.io/learningr/assets/data-transformation.pdf)

-   [Data Visualization Cheat Sheet](https://www.maths.usyd.edu.au/u/UG/SM/STAT3022/r/current/Misc/data-visualization-2.1.pdf)
